...
class TMM_Mail_Subscription {
    ...
    public static function get_string_option($name){       
        switch ($name){
            ...
            case 'info_failed_unsubscription':
                $option = esc_html__('Something went wrong, we were unable to remove your email. Please contact site admin.', 'diplomat');
                break;
    ...
    public static function save_post($post_id) {
        ...
        if (!empty($recepient_mails)){
            foreach ($recepient_mails as $email){
	            ...
	            $unsubscribe_link = '<a href=' . site_url().'?action=unsubscribe_request&email='.$email . '>' . __("Unsubscribe", 'diplomat') . '</a>';
    ...
    public static function send_email($email, $subject, $messagebody, $errors, $add_post=false){
        ...
        /* check subject */
		if (empty($subject)) {
			$subject = __("Email from subscription form", 'diplomat');
		}

		/* set message */
		$after_message = '<tr><td><br/>--------------------------------------------------------</td></tr><tr><td>' . __('This mail was sent via', 'diplomat') . ' <a href="' . site_url() . '">' . get_bloginfo('name') . '</a> ' . __('subscribtion form', 'diplomat') . '</td></tr></table>';
		$messagebody = '<table  width=100% border=0><tr><td>' . $pre_messagebody_info . '</td></tr><tr><td>' . nl2br($messagebody) . '</td></tr>' . $after_message . '</table>';
